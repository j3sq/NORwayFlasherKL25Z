...
class TeensySerial(object):
	...
	def __init__(self, port):
		self.ser = serial.Serial(port, 9600, timeout=60, rtscts=False, dsrdtr=False, xonxoff=False, writeTimeout=12,bytesize = serial.EIGHTBITS,
	...
	def write(self, s, requestConfirm=0):
		#print "Buffer Size %d",self.BUFSIZE
		if isinstance(s,int):
			s = chr(s)
		...
		while len(self.obuf) > self.BUFSIZE:
			...
			if (requestConfirm==1):
#				print "waiting conformation"
				tmpBuf=self.obuf
				self.obuf=''
				val = self.readbyte()
				if (val!=0x7C):
					print "Feedback Not Received"
		...
		if (wordmode == True):
			...
			for block in range(0,ssize,0x1000):
				...
				while retries != 0:
					...
					self.flush()
					self.write(data[block:block+0x1000],1)
					#print "Ho..."
					...
					error_msg = ""
					if (res != 75): #'K'
						print "Error; returned res = 0x%02x at block 0x%02x"%res,block
						if (res == 84): #'T'
